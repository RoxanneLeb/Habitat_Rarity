...
gisbase = os.environ['GISBASE'] = "/home/majavie/grass_sc/grass7_trunk/dist.x86_64-unknown-linux-gnu"
gisdbase = os.path.join("/home/leberro/grassdb")
location = "World_realm_5km"
mapset   = "leberro"#"rasterized_parks"
...
source = 'Cox_Olson_realm_forest' # shp file combining all the realms
#grass.run_command('v.in.ogr',flags='oe',dsn='.',lay=source,out=source,overwrite=True)
grass. message ("Extracting list of PAs")
pa_list0 = grass.read_command ('v.db.select', map=source,columns='Val3'). splitlines ()
...
n = len(pa_list)-1 # there is also a ID element!
#print pa_list[1]
...
#for pa in pa_list:
 ...
 grass.run_command('g.region',vect=pa0,res=5000,flags='d')
 grass.run_command('v.to.rast',input=pa0,out=pa0,use='cat',labelcol='Val3')
 opt3 = pa2+'= @'+pa0
 opt4 = pa2+'= round('+pa2+')'
 ...
 #r.buffer map=pa2 buff=xxx out=pa4
 #grass.run_command('r.buffer',input=pa2,output=pa4,distances=5,units='kilometers',overwrite=True)
 grass.run_command('r.null',map=pa4,null=0)
 eco_list = grass.read_command ('r.stats', input='eco_555',sort='desc'). splitlines ()
 print eco_list
 eco = eco_list[0]
 print eco
 econame = str(eco)+'.csv'
