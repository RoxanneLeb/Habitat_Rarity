...
# To select _1.tif and _2.tif sp wich are not processed yet to change reolution
###
...
birds_1_proj <- read.table('out_1/out_1_list2.txt')
...
birds_1_num_proj <- strsplit(as.character(birds_1_proj[,1]),"_")
birds_1_cat_proj <- sapply(birds_1_num_proj, function(x) {length(x) <- 3; x[3] })
...
mkdir out_1_2
cat birds_1_list_toProject.txt | xargs -I {} cp {} ../out_1_2/
